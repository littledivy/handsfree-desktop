...
function reducer(s: State, a: Action): State {
  switch (a.t) {
    case "user":
      return {
        items: [...s.items, { kind: "user", text: a.text }],
        busy: true,
      };
    case "assistant": {
      ...
      if (last?.kind === "assistant") {
        items[items.length - 1] = { kind: "assistant", text: a.text };
    ...
    case "shot":
      ...
      return {
        ...s,
        items: [...s.items, { kind: "shot", id: a.id, src: a.src }],
      };
    case "done":
      return { ...s, busy: false };
    case "error":
      ...
      for (const b of msg?.content ?? []) {
        if (b.type === "text") text += b.text;
          ...
          dispatch({
            t: "tool",
            id: String(
              b.id ?? b.toolCallId ?? JSON.stringify(b.arguments ?? b.input),
            ),
